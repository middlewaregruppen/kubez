...
type CGroup struct {
	...
	CpuStatNrPeriods     int64 `json:"cpuStatNr_periods"`
	CpuStatNrThrottled   int64 `json:"cpuStatNr_throttled"`
	CpuStatThrottledTime int64 `json:"cpuStatThrottled_time"`
	...
	MemoryLimitInBytes int64            `json:"memoryLimit_in_bytes"`
	MemoryUsageInBytes int64            `json:"memoryUsage_in_bytes"`
}
...
func getIntMapFromFile(file string) (map[string]int64, error) {
	...
	for _, ln := range lns {
		kv := strings.Split(ln, " ")
		v, _ := strconv.ParseInt(string(kv[1]), 10, 64)
		m[kv[0]] = v
	}
	return m, nil
}
